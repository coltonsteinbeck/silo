...
describe('PostgresAdapter', () => {
  ...
  describe('healthCheck', () => {
    test('returns true when database responds', async () => {
      adapter.pool.query = mock(async (): Promise<QueryResult> => ({ rows: [{ result: 1 }], rowCount: 1 }));
  ...
  describe('getUserMemories', () => {
    test('retrieves memories for user', async () => {
      ...
      adapter.pool.query = mock(async (): Promise<QueryResult> => ({ rows: mockMemories, rowCount: 2 }));
  ...
  describe('getConversationHistory', () => {
    test('retrieves conversation messages', async () => {
      ...
      adapter.pool.query = mock(async (): Promise<QueryResult> => ({ rows: mockMessages, rowCount: 2 }));
  ...
  describe('storeConversationMessage', () => {
    test('stores new message', async () => {
      const mockMessage = { id: '1', channel_id: 'chan123', user_id: 'user123', role: 'user', content: 'Hello' };
      adapter.pool.query = mock(async (): Promise<QueryResult> => ({ rows: [mockMessage], rowCount: 1 }));
