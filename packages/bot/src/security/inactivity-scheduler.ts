...
class InactivityScheduler {
    ...
    async processInactiveWarnings(): Promise<{ sent: number; failed: number }> {
        const guilds = await this.query<InactiveGuild>(
        ...
        for (const guild of guilds) {
            try {
                ...
                if (success) {
                    sent++;
                    logger.info(`Sent inactivity warning to ${guild.guild_name} (${guild.days_inactive} days)`);
                } else {
                    failed++;
    ...
    async processEvictions(): Promise<{ evicted: number; failed: number }> {
        const guilds = await this.query<GuildToEvict>(
            `SELECT * FROM get_guilds_to_evict()`
        );
        ...
        for (const guild of guilds) {
            try {
                ...
                evicted++;
                logger.info(`Evicted guild ${guild.guild_name} after ${guild.days_inactive} days of inactivity`);
            } catch (error) {
                failed++;
    ...
    async processDataDeletions(): Promise<{ deleted: number; failed: number }> {
        const guilds = await this.query<GuildToDelete>(
        ...
        for (const guild of guilds) {
            try {
                ...
                deleted++;
                logger.info(`Deleted data for guild ${guild.guild_name} (deactivated: ${guild.deactivated_at})`);
            } catch (error) {
                failed++;
    ...
    async getUpcomingActions(): Promise<{
        ...
        const guildsToEvict = await this.query<GuildToEvict>(
            `SELECT * FROM get_guilds_to_evict()`
        );
