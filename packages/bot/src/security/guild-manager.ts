...
import { Client, Guild, TextChannel, ChannelType, ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder } from 'discord.js';
import { Pool } from 'pg';
...
class GuildManager {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { 
                    ...
                    value: 'When a spot opens up, we\'ll send a notification here. You\'ll have **48 hours** to activate your slot.'
                },
                {
                    name: '🖥️ Can\'t wait?',
                    value: 'You can self-host Silo with your own database for unlimited guilds!\n' +
    ...
    async handleGuildLeave(guildId: string): Promise<void> {
        await this.query(
            `SELECT deactivate_guild($1, 'left')`,
            [guildId]
        );
        
        // Remove from waitlist if present
        await this.query(
            `DELETE FROM guild_waitlist WHERE guild_id = $1`,
            [guildId]
        );
    ...
    async evictGuild(guildId: string, sendNotification: boolean = true): Promise<void> {
                ...
                if (channel) {
                    const embed = new EmbedBuilder()
                        ...
                        .addFields(
                            {
                                name: 'Want to return?',
                                value: 'Simply use any Silo command to rejoin. If we\'re at capacity, you\'ll be added to the waitlist.'
                            },
                            {
                                name: 'Data retention',
                                value: 'Your memories and preferences are saved for 30 days. After that, they\'ll be permanently deleted.'
                            }
                        )
        ...
        await this.query(
            `SELECT deactivate_guild($1, 'inactivity')`,
            [guildId]
        );
                ...
                if (channel) {
                    const embed = new EmbedBuilder()
                        ...
                        .addFields(
                            ...
                            {
                                name: '⚠️ Important',
                                value: 'If you don\'t activate within 48 hours, your spot will go to the next server in line.'
                            }
                        )
    ...
    async updateActivity(guildId: string): Promise<void> {
        await this.query(
            `SELECT update_guild_activity($1)`,
            [guildId]
        );
    }
    ...
    async acceptWaitlistPromotion(guildId: string): Promise<boolean> {
        ...
        await this.query(
            `UPDATE guild_waitlist SET status = 'joined' WHERE guild_id = $1`,
            [guildId]
        );
