-- Migration 018: Follow-up fixes for vision quota migration
--
-- Purpose:
-- - Preserve immutable historical migration 013.
-- - Re-assert vision quota schema additions with valid IF NOT EXISTS syntax.
-- - Recreate quota functions with valid PL/pgSQL formatting and deterministic fallback ordering.

-- Ensure vision columns exist with valid syntax.
ALTER TABLE usage_tracking
  ADD COLUMN
IF NOT EXISTS vision_tokens_used INTEGER DEFAULT 0,
ADD COLUMN
IF NOT EXISTS vision_requests INTEGER DEFAULT 0;

ALTER TABLE guild_daily_usage
  ADD COLUMN
IF NOT EXISTS total_vision_tokens BIGINT DEFAULT 0;

ALTER TABLE guild_quotas
  ADD COLUMN
IF NOT EXISTS daily_vision_tokens INTEGER NOT NULL DEFAULT 20000;

ALTER TABLE role_tier_quotas
  ADD COLUMN
IF NOT EXISTS vision_tokens INTEGER NOT NULL DEFAULT 5000;

-- Ensure global default vision quotas are present.
...
UPDATE role_tier_quotas SET vision_tokens = 4000 WHERE guild_id IS NULL AND role_tier = 'trusted';
UPDATE role_tier_quotas SET vision_tokens = 1500 WHERE guild_id IS NULL AND role_tier = 'member';
UPDATE role_tier_quotas SET vision_tokens = 0 WHERE guild_id IS NULL AND role_tier = 'restricted';

-- Recreate tier quota lookup with deterministic ordering and valid IF control flow.
CREATE OR REPLACE FUNCTION get_role_tier_quota
(
  p_guild_id TEXT,
  p_role_tier TEXT
) RETURNS TABLE
(text_tokens INTEGER, images INTEGER, voice_minutes INTEGER, vision_tokens INTEGER) AS $$
BEGIN
    RETURN QUERY
    ...
    ORDER BY rtq.updated_at DESC NULLS LAST, rtq
...
-- Recreate atomic increment function with explicit vision branch.
...
(success BOOLEAN, new_total INTEGER, remaining INTEGER) AS $$
DECLARE
  v_current INTEGER;
  v_result RECORD;
BEGIN
...
RETURN QUERY
SELECT v_result.success, v_result.new_total, v_result.remaining;
RETURN;
END
IF;
...
RETURN QUERY
SELECT FALSE, v_current, GREATEST(0, p_user_limit - v_current);
END;
$$ LANGUAGE plpgsql;
