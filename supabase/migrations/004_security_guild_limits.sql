...
-- System channel or first text channel for warnings
...
CREATE INDEX IF NOT EXISTS idx_guild_registry_active ON guild_registry(is_active, deployment_mode) WHERE is_active = true;
CREATE INDEX IF NOT EXISTS idx_guild_registry_activity ON guild_registry(last_activity_at) WHERE is_active = true;
CREATE INDEX IF NOT EXISTS idx_guild_registry_deletion ON guild_registry(scheduled_deletion_at) WHERE scheduled_deletion_at IS NOT NULL;
...
member_count INTEGER DEFAULT 0,
position SERIAL,
-- Auto-incrementing for FIFO order
requested_at TIMESTAMPTZ DEFAULT NOW(),
notified_at TIMESTAMPTZ,
expires_at TIMESTAMPTZ,
-- 48h after notification to accept
status TEXT DEFAULT 'waiting' CHECK (status IN ('waiting', 'notified', 'expired', 'joined'))
);

CREATE INDEX IF NOT EXISTS idx_waitlist_status_position ON guild_waitlist(status, position) WHERE status = 'waiting';
CREATE INDEX IF NOT EXISTS idx_waitlist_expires ON guild_waitlist(expires_at) WHERE status = 'notified';
...
-- Length for analysis without storing content
...
CREATE INDEX IF NOT EXISTS idx_moderation_hash ON content_moderation_log(content_hash);
CREATE INDEX IF NOT EXISTS idx_moderation_user_action ON content_moderation_log(user_id, action_taken);
CREATE INDEX IF NOT EXISTS idx_moderation_guild ON content_moderation_log(guild_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_moderation_blocked ON content_moderation_log(action_taken, created_at DESC) WHERE action_taken = 'blocked';
...
CREATE OR REPLACE FUNCTION can_add_hosted_guild
(max_guilds INTEGER DEFAULT 5)
RETURNS BOOLEAN AS $$
BEGIN
    RETURN get_active_hosted_guild_count()
    < max_guilds;
END;
$$ LANGUAGE plpgsql;
...
()
RETURNS TABLE
(guild_id TEXT, guild_name TEXT, owner_id TEXT, member_count INTEGER, queue_position INTEGER) AS $$
BEGIN
    RETURN QUERY
    SELECT
        w.guild_id,
...
BEGIN
    ...
    SELECT
        ...
        EXTRACT(DAY FROM NOW() - g.last_activity_at)
    ::INTEGER as days_inactive,
    g.owner_id
FROM guild_registry g
WHERE g.is_active = true
  AND g.deployment_mode = 'hosted'
  AND g.last_activity_at < NOW
    () - INTERVAL '25 days'
  AND g.last_activity_at >= NOW
    () - INTERVAL '30 days'
  AND
    (g.last_warning_sent_at IS NULL OR g.last_warning_sent_at::DATE < CURRENT_DATE);
END;
$$ LANGUAGE plpgsql;
...
()
RETURNS TABLE
(guild_id TEXT, guild_name TEXT, deactivated_at TIMESTAMPTZ) AS $$
BEGIN
    RETURN QUERY
    SELECT
        g.guild_id,
...
CREATE OR REPLACE FUNCTION deactivate_guild
(p_guild_id TEXT, p_reason TEXT)
RETURNS VOID AS $$
BEGIN
    UPDATE guild_registry SET
    is_active = false,
    deactivated_at = NOW(),
    scheduled_deletion_at = NOW() + INTERVAL
    '30 days',
...
()
RETURNS TABLE
(guild_id TEXT, owner_id TEXT, guild_name TEXT) AS $$
DECLARE
v_next RECORD;
BEGIN
    SELECT *
    INTO v_next
    FROM get_next_waitlist_guild();
...
RETURN QUERY
SELECT v_next.guild_id, v_next.owner_id, v_next.guild_name;
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION delete_guild_data
(p_guild_id TEXT)
RETURNS VOID AS $$
BEGIN
...
v_count INTEGER;
BEGIN
    -- Get the target guild's position (only if waiting)
    SELECT position
