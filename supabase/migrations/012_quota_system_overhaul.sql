...
CREATE TABLE
IF NOT EXISTS role_tier_quotas
(
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4
(),
  guild_id TEXT,  -- NULL = global default
  role_tier TEXT NOT NULL CHECK
(role_tier IN
('admin', 'moderator', 'trusted', 'member', 'restricted')),
  text_tokens INTEGER NOT NULL CHECK
(text_tokens >= 0),
  images INTEGER NOT NULL CHECK
(images >= 0),
  voice_minutes INTEGER NOT NULL CHECK
(voice_minutes >= 0),
...
CREATE INDEX
IF NOT EXISTS idx_role_tier_quotas_guild ON role_tier_quotas
(guild_id);
CREATE INDEX
IF NOT EXISTS idx_role_tier_quotas_tier ON role_tier_quotas
(role_tier);

-- Pre-populate global defaults (immutable by admins, managed by system only)
INSERT INTO role_tier_quotas
  (guild_id, role_tier, text_tokens, images, voice_minutes)
VALUES
...
CREATE TABLE
IF NOT EXISTS quota_accuracy_log
(
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4
(),
...
CREATE INDEX
IF NOT EXISTS idx_accuracy_log_created ON quota_accuracy_log
(created_at DESC);
CREATE INDEX
IF NOT EXISTS idx_accuracy_log_guild ON quota_accuracy_log
(guild_id);
...
CREATE TABLE
IF NOT EXISTS quota_reset_notifications
(
  guild_id TEXT NOT NULL,
  user_id TEXT NOT NULL,
...
CREATE INDEX
IF NOT EXISTS idx_reset_notifications_exhausted ON quota_reset_notifications
(exhausted_at);
...
DROP TRIGGER IF EXISTS update_role_tier_quotas_updated_at
ON role_tier_quotas;
CREATE TRIGGER update_role_tier_quotas_updated_at 
BEFORE
UPDATE ON role_tier_quotas
FOR EACH ROW
...
DROP FUNCTION IF EXISTS get_role_tier_quota
(TEXT, TEXT);
...
(text_tokens INTEGER, images INTEGER, voice_minutes INTEGER) AS $fn_get_role_tier_quota$
BEGIN
...
DROP FUNCTION IF EXISTS increment_usage_atomic
...
(success BOOLEAN, new_total INTEGER, remaining INTEGER) AS $fn_increment_usage_atomic$
WITH ensured AS (
INSERT INTO usage_tracking
  (guild_id, user_id, usage_date)
VALUES
  (p_guild_id, p_user_id, CURRENT_DATE)
ON CONFLICT
(guild_id, user_id, usage_date) DO NOTHING
  RETURNING 1
), updated AS
(
...
INSERT INTO guild_daily_usage
  (guild_id, usage_date, total_text_tokens, total_images, total_voice_minutes)
SELECT
  p_guild_id,
  CURRENT_DATE,
...
DROP FUNCTION IF EXISTS get_accuracy_stats
(INTEGER);

CREATE OR REPLACE FUNCTION get_accuracy_stats
(p_days INTEGER DEFAULT 7)
RETURNS TABLE
(avg_ratio NUMERIC, sample_count BIGINT, std_dev NUMERIC) AS $fn_get_accuracy_stats$
SELECT
...
WHERE created_at > NOW() - (p_days || ' days')
::INTERVAL
    AND input_length > 0
    AND actual_tokens > 0;
...
DROP FUNCTION IF EXISTS cleanup_old_accuracy_logs
(INTEGER);

CREATE OR REPLACE FUNCTION cleanup_old_accuracy_logs
(p_days INTEGER DEFAULT 30)
RETURNS INTEGER AS $fn_cleanup_old_accuracy_logs$
DECLARE
  deleted_count INTEGER;
BEGIN
  DELETE FROM quota_accuracy_log WHERE created_at < NOW() - (p_days || ' days')
  ::INTERVAL;
...
DROP FUNCTION IF EXISTS cleanup_old_usage
(INTEGER);

CREATE OR REPLACE FUNCTION cleanup_old_usage
(p_days INTEGER DEFAULT 90)
RETURNS TABLE
(usage_deleted INTEGER, guild_usage_deleted INTEGER) AS $fn_cleanup_old_usage$
DECLARE
...
RETURN QUERY
SELECT v_usage_deleted, v_guild_deleted;
END;
...
DROP FUNCTION IF EXISTS get_guild_quota_stats
(TEXT);

CREATE OR REPLACE FUNCTION get_guild_quota_stats
(p_guild_id TEXT)
RETURNS TABLE
(
...
SELECT
  ...
  (SELECT COUNT(*)
  FROM quota_reset_notifications
  WHERE guild_id = p_guild_id) as pending_reset_notifications
...
DO $do_enable_rls_role_tier_quotas$
BEGIN
  IF EXISTS (SELECT 1
  FROM pg_tables
  WHERE tablename = 'role_tier_quotas' AND schemaname = 'public') THEN
  EXECUTE 'ALTER TABLE role_tier_quotas ENABLE ROW LEVEL SECURITY';
END
IF;
END $do_enable_rls_role_tier_quotas$;

DO $do_enable_rls_quota_accuracy_log$
BEGIN
  IF EXISTS (SELECT 1
  FROM pg_tables
  WHERE tablename = 'quota_accuracy_log' AND schemaname = 'public') THEN
  EXECUTE 'ALTER TABLE quota_accuracy_log ENABLE ROW LEVEL SECURITY';
END
IF;
END $do_enable_rls_quota_accuracy_log$;

DO $do_enable_rls_quota_reset_notifications$
BEGIN
  IF EXISTS (SELECT 1
  FROM pg_tables
  WHERE tablename = 'quota_reset_notifications' AND schemaname = 'public') THEN
...
DROP POLICY
IF EXISTS "Service role full access to role_tier_quotas" ON role_tier_quotas;
...
DROP POLICY
IF EXISTS "Service role full access to quota_accuracy_log" ON quota_accuracy_log;
...
DROP POLICY
IF EXISTS "Service role full access to quota_reset_notifications" ON quota_reset_notifications;
