...
ALTER TABLE usage_tracking
  ADD COLUMN
IF NOT EXISTS video_tokens_used INTEGER DEFAULT 0,
ADD COLUMN
IF NOT EXISTS video_requests INTEGER DEFAULT 0;

-- Per-guild daily aggregate tracking
ALTER TABLE guild_daily_usage
  ADD COLUMN
IF NOT EXISTS total_video_tokens BIGINT DEFAULT 0;

-- Guild-level quota ceiling for video
ALTER TABLE guild_quotas
  ADD COLUMN
IF NOT EXISTS daily_video_tokens INTEGER NOT NULL DEFAULT 20;

-- Role-tier quota for video
ALTER TABLE role_tier_quotas
  ADD COLUMN
IF NOT EXISTS video_tokens INTEGER NOT NULL DEFAULT 0;

-- Seed global defaults for existing tiers
UPDATE role_tier_quotas
SET video_tokens = 20
WHERE guild_id IS NULL AND role_tier = 'admin' AND video_tokens = 0;

UPDATE role_tier_quotas
SET video_tokens = 12
WHERE guild_id IS NULL AND role_tier = 'moderator' AND video_tokens = 0;

UPDATE role_tier_quotas
SET video_tokens = 8
WHERE guild_id IS NULL AND role_tier = 'trusted' AND video_tokens = 0;

UPDATE role_tier_quotas
SET video_tokens = 4
WHERE guild_id IS NULL AND role_tier = 'member' AND video_tokens = 0;

UPDATE role_tier_quotas
SET video_tokens = 0
WHERE guild_id IS NULL AND role_tier = 'restricted' AND video_tokens = 0;
...
BEGIN
  within_quota := check_guild_quota
(p_guild_id, p_resource, p_amount);
...
IF;
  ...
  PERFORM get_or_create_usage
(p_guild_id, p_user_id);
...
BEGIN
  ...
  VALUES
    (p_guild_id, p_user_id, quota_local_date())
  ON CONFLICT
