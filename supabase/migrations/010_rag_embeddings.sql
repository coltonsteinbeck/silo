...
CREATE INDEX
IF NOT EXISTS idx_user_memory_embedding
ON user_memory USING hnsw
(embedding vector_cosine_ops)
WITH
(m = 16, ef_construction = 64);

CREATE INDEX
IF NOT EXISTS idx_server_memory_embedding
ON server_memory USING hnsw
(embedding vector_cosine_ops)
WITH
(m = 16, ef_construction = 64);
...
CREATE INDEX
IF NOT EXISTS idx_user_memory_context_type
ON user_memory
(context_type);

CREATE INDEX
IF NOT EXISTS idx_server_memory_context_type
ON server_memory
(context_type);
...
DROP FUNCTION IF EXISTS search_user_memories_by_embedding
(UUID, vector
(1536), TEXT, INT, FLOAT);
DROP FUNCTION IF EXISTS search_user_memories_by_embedding
(TEXT, vector
(1536), TEXT, INT, FLOAT);
DROP FUNCTION IF EXISTS search_server_memories_by_embedding
(UUID, vector
(1536), TEXT, INT, FLOAT);
DROP FUNCTION IF EXISTS search_server_memories_by_embedding
(TEXT, vector
(1536), TEXT, INT, FLOAT);
DROP FUNCTION IF EXISTS search_user_memories_hybrid
(UUID, TEXT, vector
(1536), TEXT, INT);
DROP FUNCTION IF EXISTS search_user_memories_hybrid
(TEXT, TEXT, vector
...
BEGIN
  ...
  WHERE
    ...
    AND
  (p_context_type IS NULL OR um.context_type = p_context_type)
    AND um.embedding IS NOT NULL
    AND
  (1 -
  (um.embedding <=> p_embedding)) >= p_similarity_threshold
    AND
...
BEGIN
  ...
  WHERE
    ...
    AND
  (p_context_type IS NULL OR sm.context_type = p_context_type)
    AND sm.embedding IS NOT NULL
    AND
  (1 -
  (sm.embedding <=> p_embedding)) >= p_similarity_threshold
    AND
  (sm.expires_at IS NULL OR sm.expires_at > NOW
  ())
...
COMMENT ON FUNCTION search_user_memories_by_embedding
(TEXT, vector
(1536), TEXT, INT, FLOAT) IS 'Search user memories by semantic similarity using vector embeddings. Returns ranked results ordered by relevance.';
COMMENT ON FUNCTION search_server_memories_by_embedding
(TEXT, vector
(1536), TEXT, INT, FLOAT) IS 'Search server memories by semantic similarity using vector embeddings. Returns ranked results ordered by relevance.';
COMMENT ON FUNCTION search_user_memories_hybrid
(TEXT, TEXT, vector
(1536), TEXT, INT) IS 'Hybrid search combining semantic and text-based search for user memories. Falls back to ILIKE when embedding not provided.';
